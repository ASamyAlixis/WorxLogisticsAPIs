...
namespace WorxLogisticsAPIs.Controllers
{
    ...
    public class ContractsController : ApiController
    {
        ...
        [Route("api/processSalesTransaction")]
        [HttpPost]
        public async Task<Response> processSalesTransaction([FromBody] JObject data)
        {
            Response res = new Response();
            string op = (data["op"] != null) ? data["op"].ToString() : string.Empty;
            string entity = (data["entity"] != null) ? data["entity"].ToString() : string.Empty;
            ...
            if (op == string.Empty || op != "processSalesTransaction" || entity == string.Empty || userid == string.Empty || attributes == string.Empty)
            {
                res.success = false;
                res.message = "Invalid Parameters";
                res.code = -1;
        ...
        [Route("api/clearInjectedTrans")]
        [HttpPost]
        public async Task<Response> clearInjectedTrans([FromBody] JObject data)
        {
            Response res = new Response();
            string op = (data["op"] != null) ? data["op"].ToString() : string.Empty;
            string userid = (data["userid"] != null) ? data["userid"].ToString() : string.Empty;

            if (op == string.Empty || op != "clearInjectedTrans" || userid == string.Empty)
            {
                res.success = false;
                res.message = "Invalid Parameters";
                res.code = -1;
            ...
            try
            {
                using (var context = new Worx_ELogisticsEntities())
                {
                    await context.Database.ExecuteSqlCommandAsync("TRUNCATE TABLE [Wrx_SalesTrans]");
